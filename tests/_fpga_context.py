...
def out_data_to_connector_1_interface(
    bus: I2CBusOutputEnableAndData, master_line_cnfg: I2CMasterLineConfiguration
):
    map_sda = map_out_to_connector(
        bus.SDA_Output_Data, bus.SDA_Output_Enable, master_line_cnfg.SDA.channel
    )
    map_scl = map_out_to_connector(
        bus.SCL_Output_Data, bus.SCL_Output_Enable, master_line_cnfg.SCL.channel
    )
...
def out_data_to_connector_4_interfaces(
    bus0: I2CBusOutputEnableAndData,
    ...
    for con in range(4):
        enable = (
            map0[con].Output_Enable
            | map1[con].Output_Enable
            | map2[con].Output_Enable
            | map3[con].Output_Enable
        )
        data = (
            map0[con].Output_Data
            | map1[con].Output_Data
            | map2[con].Output_Data
            | map3[con].Output_Data
        )
        mask = (
            map0[con].Enable_Mask
            | map1[con].Enable_Mask
            | map2[con].Enable_Mask
            | map3[con].Enable_Mask
        )
        connector_data.append(ConnectorOverride(enable, data, mask))
    return connector_data
...
def i2c_bus_to_out_enable_and_data(bus: I2CInterface, enable_clk_stretch: bool):
    bus_out = I2CBusOutputEnableAndData(
        False,
        not bus.Data,
...
def extract_data_from_connectors(
    con0_bus: I2CInterface,
...
def extract_i2c_data_1_interface(
    cn0_rd_data: int,
    cn1_rd_data: int,
    cn2_rd_data: int,
    cn3_rd_data: int,
    ...
    cn0 = extract_data_from_connector(
        cn0_rd_data, mstr_ln_cnfg.SDA.channel, mstr_ln_cnfg.SCL.channel
    )
    cn1 = extract_data_from_connector(
        cn1_rd_data, mstr_ln_cnfg.SDA.channel, mstr_ln_cnfg.SCL.channel
    )
    cn2 = extract_data_from_connector(
        cn2_rd_data, mstr_ln_cnfg.SDA.channel, mstr_ln_cnfg.SCL.channel
    )
    cn3 = extract_data_from_connector(
        cn3_rd_data, mstr_ln_cnfg.SDA.channel, mstr_ln_cnfg.SCL.channel
    )
    bus_out = extract_data_from_connectors(
        cn0, cn1, cn2, cn3, mstr_ln_cnfg.SDA.connector, mstr_ln_cnfg.SCL.connector
    )
    return bus_out


def extract_i2c_data_4_interfaces(
    cn0_rd_data: int,
    cn1_rd_data: int,
    cn2_rd_data: int,
    cn3_rd_data: int,
    ...
    bus0 = extract_i2c_data_1_interface(
        cn0_rd_data, cn1_rd_data, cn2_rd_data, cn3_rd_data, mstr0_ln_cnfg
    )
    bus1 = extract_i2c_data_1_interface(
        cn0_rd_data, cn1_rd_data, cn2_rd_data, cn3_rd_data, mstr1_ln_cnfg
    )
    bus2 = extract_i2c_data_1_interface(
        cn0_rd_data, cn1_rd_data, cn2_rd_data, cn3_rd_data, mstr2_ln_cnfg
    )
    bus3 = extract_i2c_data_1_interface(
        cn0_rd_data, cn1_rd_data, cn2_rd_data, cn3_rd_data, mstr3_ln_cnfg
    )
...
def data_and_header_capture(
    header_word_in: I2CHeaderWord,
    ...
    header_word_out, ten_bit_addr_detected = parse_header(
        header_word_in, index, ten_bit_addr_ack
    )  #
    if reset:
...
def byte_controller(
    data_in: I2CInterface,
    transfer_settings: I2CTransferSettings,
    write_data: I2CDataType,
    abort: bool,
    direction_ready: bool,
):
    global transfer_settings_reg, write_data_reg, master_value_reg, match_divide_reg, master_state_reg, state_counter_reg, state_condition_reg, condition_md1_reg, condition_md2_reg, valid_reg, data_pre_reg
    valid = valid_reg
    data = data_pre_reg >> 1
    ...
    if master_state_reg == MasterState.Idle:
        ...
        temp_stat = (
            MasterState.StopConditionSetUp
            if transfer_settings_reg.SendStopCondition
            else MasterState.Waiting
        )
...
def i2c_master(
    i2c_bus_in: I2CInterface,
    direction_ready: bool,
    settings: WorldControllerSetting,
    write_data: typing.List[int],
    go: bool,
):
    global go_reg, i2c_master_state_reg, i2c_master_valid_reg, i2c_ready_reg, i2c_value_reg, i2c_master_ack_reg, cond_md1_reg, cond_md2_reg, rd_master_data_reg, rd_master_data_list_reg
    ...
    if i2c_master_state_reg == WorldControllerState.Idle:
        ...
        out5 = (
            WorldControllerState.Idle
            if t2
            else WorldControllerState.Waiting
            if t3
            else WorldControllerState.Data
        )
        out6 = i2c_master_valid_reg and i2c_master_ack_reg
        out7 = settings.Read
        out8 = temp1
    ...
    i2c_bus_out, data, i2c_ready_temp = byte_controller(
        i2c_bus_in, transfer_set, write_data_out, out9, direction_ready
    )
...
def i2c_slave(i2c_input: I2CInterface, return_data: typing.List[int], filter_in: int, ack_in: bool):
    global filter_input_reg, ack_in_reg, slave_state_reg, bit_counter_reg, next_state_reg, byte_counter_reg, ten_bit_addr_detected_reg, ack_requested_reg, case_out_reg, data_header_reg, ten_bit_addr_reg
    filtered_input = filter_input(filter_in, i2c_input)
    falling_edge = I2CInterface(
        filter_input_reg.Clock and not filtered_input.Clock,
        filter_input_reg.Data and not filtered_input.Data,
    )
    rising_edge = I2CInterface(
        filtered_input.Clock and not filter_input_reg.Clock,
    ...
    if slave_state_reg == I2CSlaveStates.WaitOnStartCondition:
        ...
        out10 = False
        temp = (
            I2CSlaveStates.RetrieveAck
            if (in8 and (in13 == 0))
            else I2CSlaveStates.MasterReadingData
        )
        out11 = I2CSlaveStates.WaitOnStartCondition if in9 else temp
        ...
        out10 = False
        out11 = (
            I2CSlaveStates.WaitOnStartCondition
            if in9
            else (in11 if in8 else I2CSlaveStates.RetrieveAck)
        )
...
def user_logic(
    ack_in: bool,
    device_addr: int,
    header: I2CHeaderWord,
    read_data_shft: bool,
    data_in: typing.List[int],
):
    global reg_1, reg_2
    ...
    num = join_numbers_8bit(
        join_numbers_8bit(data_in[1], data_in[2]), join_numbers_8bit(data_in[3], data_in[4])
    )
