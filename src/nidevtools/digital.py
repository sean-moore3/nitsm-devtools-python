...
def tsm_ssc_configure_trigger_signal(tsm: TSMDigital, source: str, edge: enums.DigitalEdge = enums.DigitalEdge.RISING):
...
def tsm_ssc_configure_hram(tsm: TSMDigital, hram_configuration: HRAM_Configuration = HRAM_Configuration()):
    ...
    _ssc_configure_hram_trigger(tsm.ssc, triggers_type, cycle_number, pattern_label, cycle_offset, vector_offset)
    return tsm
...
def tsm_ssc_log_hram_results(
    ...
    for cycle_informations, site_number in zip(per_site_cycle_information, tsm.site_numbers):
        ...
        if not cycle_informations or all([not cycle_information.per_pin_pass_fail for cycle_information in cycle_informations]):
            ...
            for cycle_information in cycle_informations:
                results.append(
                    [
                        ...
                        "{"
                        + ",".join(
                            [(lambda x: "P" if x is True else "F")(value) for value in cycle_information.per_pin_pass_fail]
                        )
                        + "}",
        ...
        filename = "HRAM_Results_site" + str(site_number) + "_" + datetime.now().strftime("%d-%b-%Y-%H-%M-%S") + ".csv"
...
def tsm_ssc_stream_hram_results(tsm: TSMDigital):
    ...
    per_instrument_per_site_to_per_site_lut = _ssc_calculate_per_instrument_per_site_to_per_site_lut(tsm.ssc, tsm.site_numbers)
    per_site_cycle_information = [[HistoryRAMCycleInformation() for _ in range(number_of_samples)] for _ in tsm.site_numbers]
...
def tsm_ssc_burst_pattern_pass_fail(
    ...
    _, per_instrument_pass = _ssc_burst_pattern_pass_fail(tsm.ssc, start_label, select_digital_function, timeout)
    per_site_pass = _apply_lut_per_instrument_to_per_site(initialized_array, per_instrument_to_per_site_lut, per_instrument_pass)
...
def tsm_ssc_apply_tdr_offsets_per_site_per_pin(tsm: TSMDigital, per_site_per_pin_tdr_values: typing.List[typing.List[float]]):
    (
...
def tsm_ssc_configure_time_set_compare_edge_per_site(tsm: TSMDigital, time_set: str, per_site_compare_strobe: typing.List[float]):
    (
        per_site_to_per_instrument_lut,
        instrument_count,
        max_sites_on_instrument,
...
def tsm_ssc_configure_voltage_levels(tsm: TSMDigital, vil: float, vih: float, vol: float, voh: float, vterm: float):
...
def tsm_ssc_ppmu_configure_voltage_limits(tsm: TSMDigital, voltage_limit_high: float, voltage_limit_low: float):
...
def tsm_ssc_write_sequencer_register(tsm: TSMDigital, sequencer_register: enums.SequencerRegister, value: int = 0):
...
def tsm_ssc_fetch_capture_waveform(tsm: TSMDigital, waveform_name: str, samples_to_read: int, timeout: float = 10):
...
def tsm_ssc_write_source_waveform_broadcast(
    ...
    _ssc_write_source_waveform_broadcast(tsm.ssc, waveform_name, waveform_data, expand_to_minimum_size, minimum_size)
    return tsm
...
def tsm_ssc_write_source_waveform_site_unique(
    ...
    initialized_array = [[[0 for _ in range(cols)] for _ in range(max_sites_on_instrument)] for _ in range(instrument_count)]
...
def _ssc_clock_generator_generate_clock(ssc: typing.List[SSCDigital], frequency: float, select_digital_function: bool = True):
    for _ssc in ssc:
        _ssc.session.channels[_ssc.channel_list].clock_generator_generate_clock(frequency, select_digital_function)
    return ssc


def _ssc_modify_time_set_for_clock_generation(ssc: typing.List[SSCDigital], frequency: float, duty_cycle: float, time_set: str):
    period = 1 / frequency
    for _ssc in ssc:
...
def _ssc_frequency_counter_measure_frequency(ssc: typing.List[SSCDigital]):
    ...
    for _ssc in ssc:
        per_instrument_frequencies.append(_ssc.session.channels[_ssc.channel_list].frequency_counter_measure_frequency())
...
def _ssc_burst_pattern_pass_fail(
    ...
    for _ssc in ssc:
        per_instrument_pass.append(
            list(_ssc.session.sites[_ssc.site_list].burst_pattern(start_label, select_digital_function, True, timeout).values())
        )
...
def _ssc_burst_pattern(
    ...
    for _ssc in ssc:
        _ssc.session.sites[_ssc.site_list].burst_pattern(start_label, select_digital_function, wait_until_done, timeout)
    return ssc
...
def _ssc_configure_single_level(ssc: typing.List[SSCDigital], level_type_to_set: LevelTypeToSet, setting: float):
    for _ssc in ssc:
        if level_type_to_set == LevelTypeToSet.VIL:
...
def _ssc_configure_time_set_compare_edge(ssc: typing.List[SSCDigital], time_set: str, compare_strobe: float):
    for _ssc in ssc:
        _ssc.session.channels[_ssc.channel_list].configure_time_set_compare_edges_strobe(time_set, compare_strobe)
    return ssc
...
def _ssc_ppmu_configure_voltage_limits(ssc: typing.List[SSCDigital], voltage_limit_high: float, voltage_limit_low: float):
    for _ssc in ssc:
...
def _ssc_ppmu_measure(ssc: typing.List[SSCDigital], measurement_type: enums.PPMUMeasurementType):
    ...
    for _ssc in ssc:
        per_instrument_measurements.append(_ssc.session.channels[_ssc.channel_list].ppmu_measure(measurement_type))
    return ssc, per_instrument_measurements


def _ssc_ppmu_source_current(ssc: typing.List[SSCDigital], current_level: float, current_level_range: float = 0):
...
def _ssc_fetch_capture_waveform(
    ...
    for _ssc in ssc:
        waveforms = _ssc.session.sites[_ssc.site_list].fetch_capture_waveform(waveform_name, samples_to_read, timeout)
...
def _ssc_export_opcode_trigger_signal(ssc: typing.List[SSCDigital], signal_id: str, output_terminal: str = ""):
    for _ssc in ssc:
        _ssc.session.pattern_opcode_events[signal_id].exported_pattern_opcode_event_output_terminal = output_terminal
    return ssc
    # End of Trigger #
...
def _ssc_filter_sites(ssc: typing.List[SSCDigital], desired_sites: typing.List[int]):
    ...
    for _ssc in ssc:
        channel_list_array, site_list_array, site_numbers = _arrange_channels_per_site(_ssc.channel_list, _ssc.site_list)
        ...
        if site_list:
            ssc_with_requested_sites.append(SSCDigital(_ssc.session, ",".join(channel_list), ",".join(site_list)))
...
def tsm_ssc_write_static_per_site(tsm: TSMDigital, per_site_state: typing.List[enums.WriteStaticPinState], auto_select=True):
    """
    auto_select=True, specifies this function to configures the output function as digital automatically.
    auto_select=False, if the pin is explicitly configured as digital already with the tsm_ssc_select_function().
    Without configuring as digital and auto_select as false, this function will not work as expected.
    ...
    initialized_array = [[enums.WriteStaticPinState.X for _ in range(max_sites_on_instrument)] for _ in range(instrument_count)]
...
def _ssc_calculate_per_instrument_per_site_to_per_site_lut(ssc: typing.List[SSCDigital], sites: typing.List[int]):
