...
class Session:
    ...
    def ss_connect(self, tsm: SMContext):
        if self.instrument_type == InstrumentTypes.daqmx:
            ...
            multiple_session = nidevtools.digital.pin_to_n_sessions(tsm, self.enable_pin)
    ...
    def ss_disconnect(self, tsm: SMContext):
        if self.instrument_type == InstrumentTypes.daqmx:
            ...
            multiple_session_info = nidevtools.digital.pin_to_n_sessions(tsm, self.enable_pin)
            data = nidigital.enums.WriteStaticPinState.X
